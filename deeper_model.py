...
class CNN2(nn.Module):
    def __init__(self, num_classes=5):
        ...
        self.cm_pool4 = CM_Pooling(channels=256,num_scales=3)
        self.layer4 = ConvBlock(256 * (1 + 3), 512)
        self.cm_pool5 = CM_Pooling(channels=512,num_scales=3)
        self.layer5 = ConvBlock(512 * (1 + 3), 1024)
        ...
        self.dpa4 = DualParallaxAugmentation(256)
        self.dpa5 = DualParallaxAugmentation(512)
    def forward(self, xL, xR):
       # First Dual Parallax Augmentation
        ...
        xL, xR = self.dpa4(xL, xR)
        xL = self.cm_pool4(xL)
        xR = self.cm_pool4(xR)
        xL = self.layer4(xL)
        xR = self.layer4(xR)

        xL, xR = self.dpa5(xL, xR)
        xL = self.cm_pool5(xL)
        xR = self.cm_pool5(xR)
        xL = self.layer5(xL)
        xR = self.layer5(xR)
